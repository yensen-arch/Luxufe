...
export default function TravelAccordion({ data }: TravelAccordionProps) {
  ...
  return (
    <section className="max-w-6xl mx-auto my-12 md:my-16 lg:my-24 px-4 md:px-6 lg:px-8">
      <div className="divide-y divide-gray-600 border-t border-b border-gray-600">
        {accordionData.items.map((item, idx) => (
          <div key={item.title} className="relative overflow-hidden transition-all duration-500">
            ...
            >
              <span className="text-lg md:text-xl lg:text-2xl xl:text-3xl font-arpona font-bold text-[#23263a] text-left">
                {item.title}
              </span>
              {/* Icon */}
            ...
            <div
              className={`transition-all duration-500 ease-in-out grid ${
                openIndex === idx ? "max-h-[500px] opacity-100 py-4 md:py-6 lg:py-8 px-4 md:px-6 lg:px-8" : "max-h-0 opacity-0"
              }`}
              style={{ overflow: "hidden" }}
            >
              <p className="font-inter text-slate-700 text-sm md:text-base lg:text-md xl:text-lg font-bold max-w-4xl">
                {item.content}
              </p>
            </div>
          </div>
