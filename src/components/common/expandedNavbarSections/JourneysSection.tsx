        ...
        {rightCards.map((card) => (
          <Link href={card.href} key={card.label} className="relative h-52 overflow-hidden shadow group cursor-pointer flex items-center">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-center justify-center">
              <span className="text-white text-sm font-arpona font-bold flex items-center gap-2 mt-auto mb-4">{card.label} <Image src="/luxufe-icon-button-arrow-light.svg" alt="Arrow right" width={28} height={28} /></span>
            </div>
          </Link>
        ))}
      </div>
