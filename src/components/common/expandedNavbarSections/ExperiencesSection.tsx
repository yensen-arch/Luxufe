...
const ExperiencesSection: React.FC = () => (
  <div className="flex flex-col lg:flex-row w-full h-full min-h-[400px] sm:min-h-[500px] lg:min-h-[600px]">
    ...
    <div className="w-full lg:w-1/2 bg-[#f3f3f5] relative flex flex-col justify-center items-center px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
      <div className="flex flex-col gap-4 sm:gap-6 lg:gap-8 w-full max-w-[420px] flex-1 justify-center">
        {rightCards.map((card) => (
          <div key={card.label} className="relative h-32 sm:h-40 lg:h-52 overflow-hidden shadow group cursor-pointer flex items-center">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-center justify-center">
              <span className="text-white text-xs sm:text-sm font-arpona font-bold flex items-center gap-1 sm:gap-2 mt-auto mb-2 sm:mb-4 px-2">{card.label} <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 lg:w-6 lg:h-6 flex-shrink-0" /></span>
            </div>
          </div>
        ))}
      </div>
      <button className="w-full max-w-[420px] border border-gray-400 py-2 sm:py-3 font-inter font-bold text-gray-900 hover:bg-gray-200 transition-all mt-6 sm:mt-8 lg:mt-10 flex items-center justify-center gap-2 text-xs">
        ALL LUXUFE JOURNEYS <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" />
      </button>
    </div>
  </div>
);
