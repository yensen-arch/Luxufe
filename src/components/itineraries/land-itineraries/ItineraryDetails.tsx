    ...
    <section className="bg-white border-t-2 border-gray-300">
      <div className="mx-auto flex flex-col md:flex-row">
        ...
        <div className="border-b-2 border-gray-300 w-full">
          <h2 className="text-2xl md:text-3xl px-4 md:px-8 pt-6 md:pt-8 font-arpona font-bold text-black mb-3 md:mb-4">{itinerary.itinerary_name}</h2></div>
        ...
        <div className="md:w-3/4 p-6 md:p-20 border-l-2 border-gray-300">
          <h1 className="text-3xl md:text-5xl font-arpona font-bold text-gray-900 mb-4 md:mb-6">The soul of {itinerary.destinations}</h1>
          <p className="text-sm md:text-md w-full md:w-4/5 font-inter font-bold mb-6 md:mb-8">{itinerary.overview}</p>
          {/* Special Offer Box */}
          ...
          <div className="grid w-full md:w-4/5 grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 md:mb-10">
            {itinerary.journey_highlights.map((highlight, index) => (
              <div key={index} className="flex items-start gap-3 md:gap-4">
                  ...
                  <span className="text-gray-800 font-inter font-bold text-sm md:text-md">{highlight}</span>
              </div>
            ))}
          </div>
          </div>
          {/* Itinerary Options */}
          <ItineraryOptions dailyItinerary={itinerary.daily_itinerary} />
        </div>
      </div>
    </section>
  );
