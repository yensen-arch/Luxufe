...
export default function VisitView() {
  ...
  const prev = () => changeImage(current === 0 ? journeyImages.length - 1 : current - 1);
  const next = () => changeImage(current === journeyImages.length - 1 ? 0 : current + 1);
