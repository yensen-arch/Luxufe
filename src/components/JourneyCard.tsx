...
const JourneyCard: React.FC<JourneyCardProps> = ({ category, title, description, imageUrl, link }) => {
  ...
  return (
    <div className="relative h-96 w-full group overflow-hidden cursor-pointer">
      <div 
      ...
      <div className="relative p-8 flex flex-col justify-end h-full text-white">
        ...
        <div className="transition-all duration-500 ease-in-out max-h-0 opacity-0 group-hover:max-h-48 group-hover:opacity-100 group-hover:mt-4">
          {description && (
            <p className="text-sm leading-relaxed mb-4">{description}</p>
          )}
          {link && (
            <a href={link.href} className="flex items-center gap-2 text-sm font-semibold tracking-wider hover:underline">
