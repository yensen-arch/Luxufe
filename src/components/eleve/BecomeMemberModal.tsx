    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="relative bg-white shadow-2xl max-w-6xl px-10 w-full flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="md:w-1/2 p-10 flex flex-col justify-between">
          <div>
            <h2 className="text-3xl font-arpona font-bold mb-6 text-[#23263a]">{modalData.title}</h2>
            <p className="mb-8 text-black font-inter font-bold text-sm w-6/8">
              {modalData.description}
            </p>
            <div className="mb-8 w-6/8 border-b border-gray-400">
              <h3 className="uppercase text-xs font-bold text-gray-500 mb-4 tracking-widest">{modalData.perksTitle}</h3>
          ...
          {modalData.footerText && (
            <p className="text-xs text-gray-500 mt-8 w-6/8">{modalData.footerText}</p>
          )}
        </div>
        {/* Right: Form */}
        <div className="md:w-1/2 p-10 bg-white relative flex flex-col">
          ...
          <form className="flex flex-col gap-4 mt-2">
            ...
            <button type="submit" className="mt-4 w-full bg-[#23263a] text-white py-4  font-inter font-bold text-[0.6rem] tracking-widest border border-gray-400 hover:bg-white hover:text-[#23263a] transition">{modalData.submitButtonText}</button>
          </form>
        </div>
      </div>
    </div>
