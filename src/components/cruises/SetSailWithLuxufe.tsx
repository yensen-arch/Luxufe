  ...
  return (
    <section className="py-24 bg-[#23263a]">
      <div className="max-w-7xl mx-auto px-4 text-center">
        ...
        <p className="text-lg md:text-xl font-inter font-bold text-white mb-16">Simple steps. Tailored service. Your perfect cruise awaits.</p>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-16">
          {steps.map((step, idx) => (
            <div key={idx} className="flex flex-col items-center text-center relative">
              <img src={step.image} alt={step.title} className="w-full h-56 object-cover mb-8" />
              {/* Dotted line between steps except last */}
              {idx < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 right-0 w-8 h-0.5 border-t-2 border-dotted border-white z-10" style={{transform: 'translateY(-50%)'}}></div>
              )}
              <h4 className="text-md font-inter font-bold text-white tracking-widest mb-4">{step.title}</h4>
              <p className="text-sm font-inter text-white max-w-xs">{step.description}</p>
            </div>
          ))}
        </div>
        <div className="flex justify-center mt-8">
          <button className="flex items-center gap-2 border border-white px-8 py-4 bg-transparent text-white font-inter font-bold text-xs tracking-widest hover:bg-white hover:text-[#23263a] transition-all">
