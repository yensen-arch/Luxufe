      ...
      <div className="max-w-7xl mx-auto px-4 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-16">
          {steps.map((step, idx) => (
            <div key={idx} className="flex flex-col items-center text-center relative">
              ...
              <h4 className="text-sm font-inter font-bold text-white tracking-widest mb-4">{step.title}</h4>
        ...
        <div className="flex justify-center mt-8">
          <button className="flex items-center gap-2 border border-gray-400 px-8 py-5 bg-transparent text-white font-inter font-bold text-xs tracking-widest hover:bg-white hover:text-[#23263a] transition-all">
            BOOK AN APPOINTMENT <Image src="/luxufe-icon-button-arrow-light.svg" alt="Arrow right" width={34} height={34} className="w-5 h-5 ml-2" />
          </button>
        </div>
      </div>
    </section>
